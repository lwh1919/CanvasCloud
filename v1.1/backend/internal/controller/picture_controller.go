...
import (
	"backend/internal/api/imagesearch"
...
// ProListPictureVOByPageWithCache godoc
// @Summary      带有缓存的分页获取一系列图片信息
// @Tags         picture
// @Accept       json
// @Produce      json
...
// @Success      200  {object}  common.Response{data=resPicture.ListPictureVOResponse} "查询成功"
// @Failure      400  {object}  common.Response "更新失败，详情见响应中的code"
// @Router       /v1/picture/list/page/vo/procache [POST]
func ProListPictureVOByPageWithCache(c *gin.Context) {
	...
	//获取分页查询对象
	pics, err := sPicture.ProListPictureVOByPageWithCache(&queryReq)
	if err != nil {
		common.BaseResponse(c, nil, err.Msg, err.Code)
		return
	}
